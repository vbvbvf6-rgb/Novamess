...
function ReferralLeaderboard({ me }: { me: any }) {
  ...
  const handleShare = () => {
    if (!myCode?.link) return;
    if (navigator.share) {
      navigator.share({ title: "Присоединяйся к Aura!", text: `Используй мой код: ${myCode.code}`, url: myCode.link }).catch(() => {});
    } else {
      handleCopyLink();
    }
  };
          ...
          <div className="p-5">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Пригласите друзей в Aura</p>
              </div>
              {myCode.invited > 0 && (
